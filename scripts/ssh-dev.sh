...
tmux new-session -d -s $SESSION_NAME -n "hel-d-m"
tmux send-keys -t $SESSION_NAME:0 "ssh root@hel-d-m" C-m

tmux new-window -t $SESSION_NAME -n "hel-d-w"
tmux send-keys -t $SESSION_NAME:1 "ssh root@hel-d-w" C-m

tmux new-window -t $SESSION_NAME -n "fsn-d-m"
tmux send-keys -t $SESSION_NAME:2 "ssh root@fsn-d-m" C-m

tmux new-window -t $SESSION_NAME -n "fsn-d-w"
tmux send-keys -t $SESSION_NAME:3 "ssh root@fsn-d-w" C-m

tmux new-window -t $SESSION_NAME -n "nbg-d-m"
tmux send-keys -t $SESSION_NAME:4 "ssh root@nbg-d-m" C-m

tmux new-window -t $SESSION_NAME -n "nbg-d-w"
tmux send-keys -t $SESSION_NAME:5 "ssh root@nbg-d-w" C-m
